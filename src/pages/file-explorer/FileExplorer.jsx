...
export function FileExplorer() {
  ...
  // Pulls the file tree from global state so everything stays in sync
  const [files, setFiles] = useReflexxValue(fileTree, 'files');
  ...
  // Figures out what to show - either the current folder's contents or the root level
  ...
  // Recursively adds a new folder to the correct spot in the tree
  ...
  // Recursively removes a node and all its children from the tree
  ...
  // Recursively finds a node and updates its name
  ...
  // Creates the actual folder object and saves it to the tree
  ...
  // The floating action buttons - one for creating folders, one for uploading files
  ...
  return (
    <>
      <Helmet>
        <title>Reflexx | Global File Store</title>
        <meta name="description" content="Manage your files and folders with drag-and-drop uploads" />
      </Helmet>
      <div className="relative min-h-screen bg-bg-canvas pb-24 transition-colors duration-300">
        <BreadCrumb path={pathSequence} basePath="/file-explorer" />
        ...
        <SpeedDial
          ...
          hideIcon={() => <Plus size={22} className="text-black rotate-45" />}
